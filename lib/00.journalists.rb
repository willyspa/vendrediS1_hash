...
puts "le tableau contient #{handle_twitter.length} journalists"

puts "-"*10

puts handle_twitter.select { |log| !!/[0-9]/.match(log) }

puts "-"*10

i =0
handle_twitter.each do |journalist|

  if journalist.include?("1") || journalist.include?("2") || journalist.include?("3") || journalist.include?("4") || journalist.include?("5") || journalist.include?("6") || journalist.include?("7") || journalist.include?("8") || journalist.include?("9")
    i+=1
...
puts "Il y a #{i} handles qui contiennent des chiffres"
...
for i in 0..handle_twitter.length
 ...
 puts "il y a #{count} handle_twitter avec des nombres"

 puts "-"*10
tour = 0
for i in 0..handle_twitter.length

  if handle_twitter[i].to_s.downcase=~/(aude)/
